...
> [!WARNING]
> I haven't uploaded anything other than results as I'm currently writing a conference paper on it. Once it is accepted, I will upload all the code for the filters, supervisor, drone dynamics, trajectory and other models.
...
health_score  = 0.5·score_rejection + 0.3·score_nis + 0.2·score_quality
...
degraded". A **minimum weight floor** for RSSI (`RSSI_MIN_WEIGHT_FLOOR ≈ 0.12`)
...
| Label | Condition |
|---|---|
| 🟦 `GPS-dominant` | `w_gps ≥ MODE_DOMINANT_THRESH` (0.75) |
| 🟧 `RSSI-dominant` | `w_rssi ≥ MODE_DOMINANT_THRESH` (0.75) |
| 🟩 `Balanced Blend` | neither dominant |
| 🟥 `DR` | **both** health scores `< DR_HEALTH_THRESH` (0.15) |
...
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Straight-Line Climb | 0.30 | 1.12 | 99.7% | 3.07 | 63.9% | 0.0% | 36.1% | 0.0% | 0.58 |
| Helical Turn | 0.29 | 1.03 | 99.8% | 2.57 | 60.9% | 0.0% | 39.1% | 0.0% | 0.58 |
| Hover-Loiter | 0.28 | 1.00 | 99.7% | 2.65 | 61.1% | 0.0% | 38.9% | 0.0% | 0.58 |
| Figure-8 Aggressive | 0.29 | 2.04 | 93.8% | 3.22 | 64.5% | 0.0% | 35.5% | 0.0% | 0.56 |
...
|---|---|:---:|:---:|
| **Filter comparison** | Supervisor (fused) | **1.03** | 99.8% |
| | GPS-only | 1.38 | 100.0% |
| | RSSI-only | 1.36 | 98.3% |
| **Gating** | With gating | **1.03** | 99.8% |
| | Without gating | 17.16 | 72.9% |
| **Blending** | With blending (INV_COV) | 1.03 | 99.8% |
| | Without blending (GPS-primary) | 1.02 | 99.7% |

![Ablation summary](results/ablation_summary.png)

Two takeaways jump out: **Mahalanobis gating is doing almost all of the heavy
lifting against outliers** — turn it off and RMSE blows up ~17× — and the fused
Supervisor beats *either* single filter running alone, confirming the adaptive
blend is extracting real information from both sensors rather than just
picking a winner.
...
Live "True vs. Fused" trajectory animations (North-East plane) under the **Combined
Attack** scenario, one per trajectory type — the fused (red, dashed) track should
stay glued to the true (black, solid) track even as the attack kicks in halfway
through the run.
...
<td align="center"><b>Figure-8 Aggressive</b><br><img src="results/Figure-8_Aggressive_dual_trajectory_animation.gif" width="360"></td>
</tr>
</table>

For the full **4-panel real-time dashboard** (Real / IMU+RSSI / IMU+GPS /
Supervisor-fused, with a live health/mode status bar), run:

```bash
python main_supervisor.py
```

which opens two interactive matplotlib windows (Linear-Climb and Helical-Climb)
instead of the static GIFs above.
